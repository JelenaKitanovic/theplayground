...
class BasicMatchingAlgorithmServiceService implements MatchingAlgorithmServiceInterface
{
    ...
    private function calculateMatchingScoreForStrengths(array $strengths1, array $strengths2): int
    {
        ...
        return count($matchingStrengths) >= self::MIN_NUMBER_OF_MATCHING_STRENGTHS ? self::MATCHING_STRENGTHS_SCORE : 0;
    }

    private function calculateMatchingScoreForGoals(string $goal1, string $goal2): int
    {
        return $goal1 === $goal2 ? self::MATCHING_GOALS_SCORE : 0;
    }

    private function calculateMatchingScoreForIdealPartner(string $idealPartner1, string $idealPartner2): int
    {
        return $idealPartner1 === $idealPartner2 ? self::MATCHING_IDEAL_PARTNERS_SCORE : 0;
    }

    private function calculateMatchingScoreForAge(string $age1, string $age2): int
    {
        return abs($age1 - $age2) <= self::MAX_AGE_DIFFERENCE_IN_YEARS ? self::MATCHING_AGE_SCORE : 0;
    }
}
